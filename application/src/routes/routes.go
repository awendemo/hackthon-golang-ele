...
func WriteContent(w http.ResponseWriter, v []byte, code int) {
...
func WriteText(w http.ResponseWriter, v string, code int) {
	w.Header().Set("Content-Length", strconv.Itoa(len(v)))
	w.Header().Set("Content-Type", applicationJson)
	w.WriteHeader(code)
	w.Write([]byte(v))
...
func WriteXml(w http.ResponseWriter, v interface{}, code int) {
	content, err := xml.Marshal(v)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
